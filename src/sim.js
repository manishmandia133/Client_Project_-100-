    ...
    for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
            ...
            if (d < 100) {
                ...
                ctx.strokeStyle = `rgba(200,242,58,${0.05 * (1 - d/100)})`;
                ctx.lineWidth = 0.5;
                ctx.stroke();
            }
        }
...
function render() {
    ...
    const addTokens = (str, cls, isY = false, pumpIdx = 0) => {
    ...
    addTokens(x, 'token P');
    for (let pump = 0; pump < state.n; pump++) addTokens(y, '', true, pump);
    addTokens(z, 'token D');
    $wrap.appendChild(frag);

    // Animate in
    gsap.fromTo('.token', { scale: 0, opacity: 0 }, {
        scale: 1, opacity: 1, duration: 0.35, stagger: 0.02, ease: 'back.out(2)'
    });
    ...
    // Stage border flash on unstable
    const stage = document.querySelector('.sim-stage');
    if (!stable) {
        gsap.fromTo(stage, { outlineColor: 'rgba(255,61,61,0)' },
            { outlineColor: 'rgba(255,61,61,0.4)', outlineWidth: '2px', outlineStyle: 'solid',
